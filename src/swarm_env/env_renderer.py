...
class GuiSR(TopDownView):
    ...
    def __init__(
        ...
        self._drones_commands: Union[
            Dict[DroneAbstract, Dict[Union[str, Controller], Command]], Type[None]
        ] = None
        if self._drones:
        ...
        self._visu_noises = VisuNoises(
            playground_size=playground.size, drones=self._drones
        )

        self.recorder = ScreenRecorder(
            self._size[0], self._size[1], fps=30, out_file=filename_video_capture
        )
    ...
    def on_update(self):
        ...
        if (
            self._rescued_number == self._total_number_wounded_persons
            and self._rescued_all_time_step == 0
        ):
    ...
    def on_mouse_release(self, x: int, y: int, button: int, modifiers: int):
        self._mouse_measure.on_mouse_release(
            x, y, button, enable=self._use_mouse_measure
        )
